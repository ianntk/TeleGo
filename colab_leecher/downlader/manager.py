...
from colab_leecher.downlader.torrent import torrent_Download, get_Torrent_Name
        ...
        for i, link in enumerate(source):
            try:
                if is_google_drive(link):
                    ...
                    torrent_msg = (
                        f"<b>🧲 PLEASE WAIT ⌛</b>\n\n"
                        f"__Connecting to peers for__\n\n"
                        f"<code>{link[:120]}</code>"
                    )
                    try:
                        await MSG.status_msg.edit_text(
                            text=torrent_msg + sysINFO(), reply_markup=keyboard()
                        )
                    except Exception as e1:
                        print(f"Couldn't Update text ! Because: {e1}")
                    Aria2c.link_info = False
                    await torrent_Download(link, i + 1)
                else:
...
async def calDownSize(sources):
    ...
    for link in natsorted(sources):
        if is_google_drive(link):
            ...
            # Torrent total size unknown until metadata is fetched — skip pre-calculation
            pass
        else:
            pass
...
async def get_d_name(link: str):
    ...
    if is_google_drive(link):
        ...
        Messages.download_name = get_Torrent_Name(link)
    else:
