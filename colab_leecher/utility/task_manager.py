...
async def taskScheduler():
    ...
    # ── Resolve the dump channel peer via raw MTProto so pyrogram caches it.
    # Required on first run or after session deletion — avoids "Peer id invalid".
    try:
        await colab_bot.get_chat(DUMP_ID)
    except Exception:
        try:
            from pyrogram.raw import functions, types as _rtypes
            _bare_id = int(str(abs(DUMP_ID))[3:])   # strip leading -100
            await colab_bot.invoke(
                functions.channels.GetChannels(
                    id=[_rtypes.InputChannel(channel_id=_bare_id, access_hash=0)]
                )
            )
            logging.info(f"Dump channel peer resolved via raw API.")
        except Exception as _e:
            logging.error(f"Could not resolve dump channel {DUMP_ID}: {_e}")
